...
class PurchaseOrder(models.Model):
    ...
    team_id = fields.Many2one(
        comodel_name="purchase.team", string="Purchase Type", domain="[('company_id', '=', company_id)]", ondelete="restrict",tracking=True
    )
    ...
    def button_confirm(self):
        for order in self:
            if not order.team_id:
                raise ValidationError("Kindly fill the purchase type.")
...
class PurchaseReport(models.Model):
    ...
    type_id = fields.Many2one("purchase.team", string="Purchase Type")

    def _select(self):
        return super(PurchaseReport, self)._select() + ", po.team_id as type_id"

    def _group_by(self):
        return super(PurchaseReport, self)._group_by() + ", type_id"